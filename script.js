...
async function startCamera() {
  ...
  try {
    if (video.srcObject) {
      ...
      startStopBtn.style.background = "linear-gradient(135deg, #ff3cac 0%, #784ba0 50%, #2b86c5 100%)";
    ...
    startStopBtn.style.background = "linear-gradient(135deg, #32ff7e 0%, #00b894 50%, #00cec9 100%)";
  ...
  async function detectFrame() {
    ...
    predictions.forEach(pred => {
      if (pred.score > 0.6 && isDangerous(pred)) {
        ...
        video.style.filter = "sepia(1) hue-rotate(-20deg) saturate(2) brightness(1.1)";
        setTimeout(() => {
...
function toggleDetection() {
  if (detectBtn.innerText === "Start Detection") {
    ...
    detectBtn.style.background = "linear-gradient(135deg, #32ff7e 0%, #00b894 50%, #00cec9 100%)";
    runDetection();
  } else {
...
function isDangerous(prediction) {
  const closeObjects = ["person", "car", "bus", "truck", "motorcycle", "bicycle"];
...
async function togglePiP() {
  try {
    ...
    if (document.pictureInPictureElement) {
      ...
      pipBtn.style.background = "linear-gradient(135deg, #ff3cac 0%, #784ba0 50%, #2b86c5 100%)";
      await document.exitPictureInPicture();
  
    } else {
      approval.style.display = "block";
      pipBtn.style.background = "linear-gradient(135deg, #32ff7e 0%, #00b894 50%, #00cec9 100%)";
